...
def get_custom_one_hot_of_activity(x, max_leucocytes, max_lacticacid):
    ...
    one_hot = np.zeros(18, dtype=np.float32)
    one_hot[ret[0]] = ret[1]

    one_hot[15] = min(x['Leucocytes'], max_leucocytes) / max_leucocytes
    if np.isnan(one_hot[15]):
        one_hot[15] = -1

    one_hot[16] = x['CRP']
    if np.isnan(one_hot[16]):
        one_hot[16] = -1

    one_hot[17] = min(x['LacticAcid'], max_lacticacid) / max_lacticacid
    if np.isnan(one_hot[17]):
        one_hot[17] = -1
