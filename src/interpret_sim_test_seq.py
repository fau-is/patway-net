...
for t in range(0, 12):
    for idx, value in enumerate(seq_features):
        ...
        x, out, h_t, out_coef = model.plot_feat_seq_effect(idx, torch.from_numpy(x_seq_final[1, t, idx].reshape(1, 1, 1)).float())  # -1
        ...
        """
        print("h_t")
        print(h_t.reshape(1,-1))
        print("out_coef")
        print(out_coef.reshape(1,-1))
