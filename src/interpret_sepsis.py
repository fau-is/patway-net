...
# (1) Sequential features (2 time steps, no history)
...
for idx, feature in enumerate(seq_features):
    ...
    plt.show()
    plt.draw()
    fig1.savefig(f'../plots/sepsis/seq_feat_diffs_{feature}.pdf', dpi=100)
    plt.close(fig1)
...
for idx, value in enumerate(static_features):
    ...
    plt.show()
    plt.draw()
    fig1.savefig(f'../plots/sepsis/stat_feat_{value}.pdf', dpi=100)
    plt.close(fig1)
...
for idx, value in enumerate(seq_features):
    ...
    if value == "CRP":  # or value == "LacticAcid" or value == "CRP":
        ...
        # sorted_x = x[sorted_indices]
        # sorted_out = out[sorted_indices]
        # plt.plot(sorted_x, sorted_out, linewidth=3, color='steelblue')  # + 0.75
        ...
        plt.show()
        plt.draw()
        fig1.savefig(f'../plots/sepsis/seq_feat_{value}.pdf', dpi=100)
        plt.close(fig1)

# (3.1) Print sequential features (global, with history)
for t in range(0, 11):
    for idx, value in enumerate(seq_features):
        plt.rcParams["figure.figsize"] = (10, 5)
        plt.rc('font', size=16)
        ...
        if value == "CRP":  # or value == "LacticAcid" or value == "CRP":
            x, out, h_t, out_coef = model.plot_feat_seq_effect(idx, torch.from_numpy(
                x_seqs_final[:, 0:t+1, idx].reshape(-1, t+1, 1)).float(), history=True)

            # x, out, h_t, out_coef = model.plot_feat_seq_effect(idx, torch.from_numpy(
            #    x_seqs_final[:, t, idx].reshape(-1, 1, 1)).float(), history=False)
            ...
            plt.show()
            plt.draw()
            fig1.savefig(f'../plots/sepsis/seq_feat_{value}_{t}.pdf', dpi=100)
            plt.close(fig1)
    ...
    if value in seq_features_rel:
        for t in range(0, 11):
            x, out, h_t, out_coef = model.plot_feat_seq_effect(
                idx, torch.from_numpy(x_seqs_final[case, t, idx].reshape(1, 1, 1)).float(), history=False)
            x = x.detach().numpy().squeeze()
            out = out.detach().numpy()
        ...
        # plt.ylim(-0.11, 0.21)
        plt.plot(list(range(1, 12)), effect_feature_values[idx], label=value, linestyle='dashed', linewidth=2,
...
plt.show()
plt.draw()
fig1.savefig(f'../plots/sepsis/seq_feat_case_{case}_no_hist.pdf', dpi=100)
plt.close(fig1)

# (4.1) Print sequential features (local, with history)
effect_feature_values = []
case = 251
colors = ['steelblue', 'olivedrab', 'crimson', 'grey', 'yellow', 'lightskyblue', 'darkmagenta', 'darkorange']
plt.gca().set_prop_cycle(color=colors)
...
seq_features=['Leucocytes', 'CRP', 'LacticAcid', 'ER Registration', 'ER Triage', 'ER Sepsis Triage',
                    ...
                    'Release E']
seq_features_rel = ['CRP', 'LacticAcid', 'ER Registration', 'ER Triage', 'ER Sepsis Triage',
                     'IV Liquid', 'IV Antibiotics']
...
plt.show()
plt.draw()
fig1.savefig(f'../plots/sepsis/seq_feat_case_{case}.with_hist.pdf', dpi=100)
plt.close(fig1)
...
if number_interactions_seq > 0:
    for idx in range(0, number_interactions_seq):
        ...
        plt.show()
        plt.draw()
        fig1.savefig(f'../plots/sepsis/interaction_{interactions_seq[idx][0]}-{interactions_seq[idx][1]}.pdf', dpi=100)
        plt.close(fig1)


# (6) Print sequential feature (local, history)
# (4.1) Print sequential features (local, with history)
case = 251
colors = ['steelblue']
plt.gca().set_prop_cycle(color=colors)
...
for idx, value in enumerate(seq_features):
    ...
    if value in seq_features_rel:
        ...
        # plt.plot(list(range(1, 12)), effect_feature_values, label=value, linestyle='dashed', linewidth=2,
        #         marker='o', markersize=6)
        plt.plot(list(range(1, 12)), effect_feature_values, '--', linewidth=0.6, color="grey")
        plt.colorbar(label='Feature value')

        steps = list(range(1, 12))
        for i in range(len(steps)):
            plt.annotate(round(data_feature_values[i], 3), (steps[i], effect_feature_values[i] + 0.01))

        plt.axhline(y=0, color='grey', linewidth=0.6)
        plt.xlabel("Time step", fontsize=16)
        plt.ylabel("Feature effect on model output", fontsize=16)
        plt.title(f"{value} effect over time of patient pathway 18", fontsize=16)
        fig1 = plt.gcf()
        # plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.07), ncol=7, fontsize=16)
        # plt.legend(loc='lower right', title='Sequential feature')  # adjust based on plot
        plt.xticks(np.arange(1, 12, 1))
        ...
        plt.show()
        plt.draw()
        fig1.savefig(f'../plots/sepsis/seq_feat_case_{case}_single.with_hist.pdf', dpi=100)
        plt.close(fig1)