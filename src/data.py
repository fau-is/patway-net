...
def get_production_data(max_len, min_len):
    # https://dl.acm.org/doi/abs/10.1145/3301300
    # https://github.com/irhete/predictive-monitoring-benchmark

    ds_path = '../data/Production.csv'

    static_features = ['Work_Order_Qty']  # Rework, 'Part_Desc_',
    seq_features = ['Resource.1', 'Report_Type', 'Qty_Completed', 'activity_duration']  # 'Resource', Activity, Qty_for_MRB

    df = pd.read_csv(ds_path, sep=";")
    df['Complete Timestamp'] = pd.to_datetime(df['Complete Timestamp'])
    df['Part_Desc_'] = df['Part_Desc_'].astype('category')
    df['Resource.1'] = df['Resource.1'].astype('category')
    ...
    # Remove 'Activity', 'Part_Desc_', 'Resource' as > 25 categories
    df = df.drop(['index', 'timesincemidnight', 'timesincelastevent',
                  'timesincecasestart', 'event_nr', 'month', 'weekday', 'hour', 'open_cases',
                  'Rework', 'Qty_for_MRB', 'Activity', 'Part_Desc_', 'Resource'], axis=1)

    max_order = max(df['Work_Order_Qty'])
    df['Work_Order_Qty'] = df['Work_Order_Qty'].apply(lambda x: x / max_order)
    max_duration = max(df['activity_duration'])
    df['activity_duration'] = df['activity_duration'].apply(lambda x: x / max_duration)
    max_qty = max(df['Qty_Completed'])
    df['Qty_Completed'] = df['Qty_Completed'].apply(lambda x: x / max_qty)

    not_transform_cols = ['Case ID', 'Work_Order_Qty', 'label', 'activity_duration', 'Qty_Completed', 'Complete Timestamp']
    df = pd.get_dummies(data=df, columns=[col for col in df.columns if col not in not_transform_cols])
