...
def get_sepsis_data_test(target_activity, max_len, min_len):
    ...
    act2int = dict(zip(seq_features, range(len(seq_features))))
    ...
    for case in df['Case ID'].unique():
        ...
        for _, x in df_tmp.iterrows():
            ...
            if after_registration_flag:
                x_seqs[-1].append(util.get_one_hot_of_activity_sepsis_test(x, max_leucocytes, max_lacticacid, max_crp, act2int))
