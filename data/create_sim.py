...
df_main = pd.DataFrame(columns =['Case ID', 'Activity', 'Timestamp', 'Gender', 'Foreigner', 'Age', 'BMI', 'CRP', 'LacticAcid', 'Label'])
...
for idx in range (0, num_cases):
    ...
    concat_case = np.vstack((caseid, acts, tmstmp, gender, foreigner, age, bmi, crp_final, lacticacid_final, label)).transpose()
    
    # Create case
    df_temp = pd.DataFrame(data=concat_case, columns =['Case ID', 'Activity', 'Timestamp', 'Gender', 'Foreigner', 'Age', 'BMI', 'CRP', 'LacticAcid', 'Label'])
...
df_main.to_csv('Simulation_data.csv')
